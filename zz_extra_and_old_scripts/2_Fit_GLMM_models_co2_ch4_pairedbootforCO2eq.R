...
#TEST plots ----
bt_boot_simple_df %>% 
  filter(ghgspecies=="ch4", term=="status") %>% 
  ggplot(aes(x=status, y=emmean_bt))+
  geom_violin(scale="width")+
  geom_boxplot()


bt_boot_simple_df %>% 
  filter(ghgspecies=="co2", term=="status") %>% 
  ggplot(aes(x=status, y=emmean_bt))+
  geom_violin(scale="width")+
  geom_boxplot()
...
data4models %>% 
  ...
  ggplot(aes(x=status, y=dailyflux_trans))+
  geom_violin(scale="width")+
  geom_boxplot()

Anova(complexmodel_list_allghg$DU_ch4)

gwp_complex_df %>% 
  filter(casepilot=="CA") %>% 
  filter(ghgspecies=="gwp20", term=="status") %>% 
  ggplot(aes(x=status, y=emmean_bt))+
  geom_violin(scale="width")+
  geom_boxplot()


head(bt_boot_simple_df
     )


test<-data4models %>% 
  filter(ghgspecies=="co2") %>% 
  group_by(casepilot, status, season, subsite, vegpresence) %>% 
  summarise(N=sum(!is.na(dailyflux))) %>% 
  filter(N<4) %>% arrange(N)



#CHECK SE from bootstraped emmeans vs SE from model. IF my bootstraped SE is much lower than the model one, it may indicate that we are overconfident with our bootstrap apporach. 
#CHECK with RI_co2
mod<- simplemodel_list_allghg$RI_co2
emmeans(mod,specs = ~status,data=mod$frame)
summarise_emm(boot_simple_df) %>% filter(term=="status", ghgspecies=="co2")


mod<- simplemodel_list_allghg$RI_ch4
emmeans(mod,specs = ~status,data=mod$frame)
summarise_emm(boot_simple_df) %>% filter(term=="status", ghgspecies=="ch4")
...
## OLD APPROACH:: 3.3. EMEANS & post-hoc (bootstraped)------
